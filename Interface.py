...
class Ui_mainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self, mainWindow):
        ...
        self.listView_result.setGeometry(QtCore.QRect(570, 300, 200, 30))
    ...
    def Predection(self):
        ...
        path = self.lineEdit_path.displayText()
        if self.radioButton_image.isChecked():
            s = Solve(MODEL_PATH, image=path)
            result = str(s.solve_one())
            self.lineEdit_result.setText(result)

        if self.radioButton_folder.isChecked():
            s = Solve(path=MODEL_PATH)
            s.solve_excel()
            result = 'E:/chest_xray/code/Picture_set.xls'
            self.lineEdit_result.setText(result)
